...
fn main() {
    ...
    let bpf_target = match arch.as_str() {
        ...
        _ => {
            eprintln!("Warning: Unknown architecture '{}', defaulting to bpfel-unknown-none", arch);
            "bpfel-unknown-none"
        }
    };
    
    println!("cargo:warning=Building eBPF for target: {} (host arch: {})", bpf_target, arch);
    
    let ebpf_dir = PathBuf::from("../honeybeepf-ebpf");
    
    // Use a separate target directory for eBPF to avoid file lock conflicts
    let ebpf_target_dir = PathBuf::from(env::var("HOME").unwrap())
        .join("cargo-target-ebpf");
    ...
    let ebpf_binary = ebpf_target_dir
        .join(format!("{}/release/honeybeepf", bpf_target));
    
    let out_file = out_dir.join("honeybeepf");
    std::fs::copy(&ebpf_binary, &out_file)
